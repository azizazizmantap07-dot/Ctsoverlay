...
/**
 * OCR skrip Cyrillic (Rusia/Ukraina/dll) via Tesseract (bukan ML Kit).
 *
 * KENAPA: ML Kit Text Recognition tidak menyediakan model on-device untuk
 * skrip Cyrillic sama sekali (lihat javadoc di
 * {@link OcrTranslateHelper#recognizeInternal}). Kalau layar berisi teks
 * Rusia/Ukraina, recognizer Latin sering salah-baca atau menghasilkan
 * deteksi bahasa "und", sehingga translate dilewati.
 *
 ...
 *
 * Model bahasa (rus.traineddata, ~1.4 MB, dari tessdata_fast) di-BUNDLE
 ...
 */
public final class TesseractCyrillicRecognizer {
    ...
    private TesseractCyrillicRecognizer() {}

    /**
     * Pastikan rus.traineddata sudah ada di penyimpanan privat app.
    ...
    /**
     * Jalankan OCR Cyrillic secara SINKRON. Harus dipanggil dari background
    ...
    public static List<OcrBlock> recognize(Context context, Bitmap bitmap) {
        ...
        try {
            ...
            if (!initOk) {
                Log.w(TAG, "Init Tesseract Cyrillic gagal (tessdata path=" + tessRoot.getAbsolutePath() + ")");
                return result;
            }
            ...
            if (iterator != null) {
                ...
                if (confCount > 0) {
                    ...
                    if (avgConf < 75.0f || confCount < 1) {
                        Log.d(TAG, "OCR Cyrillic (Tesseract) dibuang: avgConf="
                                + String.format("%.1f", avgConf) + " baris=" + confCount
                                + " (kemungkinan hallucinasi)");
                        result.clear();
                    } else {
                        Log.d(TAG, "OCR Cyrillic (Tesseract) avgConf="
                                + String.format("%.1f", avgConf) + " baris=" + confCount);
                    }
                }
            }
        } catch (Exception e) {
            Log.w(TAG, "OCR Tesseract Cyrillic gagal: " + e.getMessage());
