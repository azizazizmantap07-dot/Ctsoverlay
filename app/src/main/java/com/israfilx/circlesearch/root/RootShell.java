...
public final class RootShell {
    ...
    /**
     * Ambil screenshot layar saat ini via root `screencap`, simpan ke
     * targetPath, lalu chmod 644 supaya bisa dibaca oleh proses app
     * (yang berjalan di UID normal, bukan UID 0).
     *
     * @param targetPath path absolut tujuan file PNG (mis. di cache
     *                   internal app sendiri, agar tidak perlu izin
     *                   storage tambahan)
     * @return true bila file berhasil dibuat dan bisa dibaca
     */
    public static synchronized boolean screencapToFile(String targetPath) {
        if (!available && !open()) {
            Log.e(TAG, "Root tidak tersedia, screencap dibatalkan");
            return false;
        }
        // Hapus dulu file lama supaya tidak salah baca hasil capture sebelumnya
        // bila screencap kali ini gagal.
        exec("rm -f " + targetPath);
        exec("screencap -p " + targetPath);
        exec("chmod 644 " + targetPath);

        String check = exec("[ -s " + targetPath + " ] && echo EXISTS || echo MISSING");
        boolean ok = check != null && check.contains("EXISTS");
        Log.d(TAG, "screencapToFile(" + targetPath + ") -> " + ok);
        return ok;
    }
}
